...
export default function InstallNudge() {
  const { isInstalled, platform, isMobile, isIOSSafari, promptInstall, canPromptInstall } = useInstallPrompt();
      ...
      <div className="bg-slate-900 rounded-2xl px-4 py-4 shadow-xl flex gap-3">
        ...
        <div className="flex-1 min-w-0">
          ...
          {platform === "ios" && isIOSSafari ? (
            ...
            <p className="text-xs text-slate-400 mt-0.5 leading-relaxed">
              Open in <span className="text-white font-semibold">Safari</span> then tap <Share className="inline w-3.5 h-3.5 mb-0.5" /> to add to your home screen
            </p>
          ) : canPromptInstall ? (
            <button
